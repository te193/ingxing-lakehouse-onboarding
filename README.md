...
Codex 只做最小定位：确认 dataset、API Path、GET/POST、迁移状态、已有成功代码或产物，以及用户当前要开始、检查、修复还是继续哪一步。台账和成功代码已经明确的内容直接复用，不重复询问。

Step 0只检查下一步真正需要的阻塞项：请求和分页在第1步确认，完整返回Schema在第2步确认，DWD粒度和业务键在第3步确认，展开与去重在第4步确认，依赖和对账边界在第5步确认。只有全新接口、台账无法定位、证据冲突或用户要求端到端设计复核时，才展开完整确认单。
...
输出包括调度参数、节点依赖、发布顺序和自动数据对账结果。

当用户确认采集或DWD节点运行成功，或要求“检测数据”“数据对账”时，Codex必须直接执行只读对账：优先通过MaxCompute MCP读取新的EXT/DWD结果，并通过`opt-lyt-db`的只读MCP查询迁移台账中对应的现有MySQL ODS/DWD表。若当前会话没有暴露某个MCP，但Skill提供了官方只读查询助手，可以使用该助手作为连接回退。

不能只生成一份对账SQL并要求用户自行执行。只有必要的只读连接确实不可用时，才把SQL作为回退文件，同时明确说明尚未完成实际对账。

对历史大表应先检查元数据和索引，再按同一业务日期、批次或快照边界执行有界查询。至少比较父记录数、子记录数、理论展开行数、DWD行数、业务键去重/空值和维表匹配率；采集时点不同时必须量化差异并标注为同日近似对账，不能宣称逐行一致。

对账结果默认使用[第5步精简报告模板](references/reconciliation-report.md)直接在会话中展示：先给结论，再给一张核心指标简表。只有发现异常时才展开样本和原因；除非用户明确要求，否则不生成额外文件。
